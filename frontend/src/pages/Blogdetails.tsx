...
<h3 className="text-sm text-gray-500"> {dayjs(data.createdAt).format("DD MMMM YYYY")}</h3></div>
          </div>
     <hr className="sm:w-2/4 w-80 bg-gray-500 h-0.3 mt-2 mb-4"/>
     <div className="flex justify-between sm:w-2/4 w-80">
<div className="flex gap-2 items-center ">
  {clapped?<PiHandsClappingFill size={23} color="gray" onClick={()=>{setClapped(!clapped);
    setClaps(claps-1)
  }} />:<PiHandsClappingBold size={23} color="gray" onClick={()=>{setClapped(!clapped); setClaps( claps+1)}}/>}
...
<IoIosMore   size={23} color="gray"/>
     ...
     <div className=" h-96  overflow-hidden" > <img src={data.blogimage} alt="" className=" h-full w-full " /></div>
